...
namespace Rte_solver_kernels_cuda_rt
{
                  ...
                  const Float* inc_flux_dir, const Float* mu0, Float* gpt_flux_dir)
    {
        ...
        apply_BC_kernel<<<grid_gpu, block_gpu>>>(ncol, nlay, top_at_1, inc_flux_dir, mu0, gpt_flux_dir);
    }
    ...
    void apply_BC(const int ncol, const int nlay, const Bool top_at_1, Float* gpt_flux_dn)
    {
        ...
        dim3 grid_gpu(grid_col);
        dim3 block_gpu(block_col);
        apply_BC_kernel<<<grid_gpu, block_gpu>>>(ncol, nlay, top_at_1, gpt_flux_dn);
    }
    ...
    void apply_BC(const int ncol, const int nlay, const Bool top_at_1, const Float* inc_flux_dif, Float* gpt_flux_dn)
    {
        ...
        dim3 grid_gpu(grid_col);
        dim3 block_gpu(block_col);
        apply_BC_kernel<<<grid_gpu, block_gpu>>>(ncol, nlay, top_at_1, inc_flux_dif, gpt_flux_dn);
    }
            ...
            const Float* sfc_src_jac, Float* flux_up_jac)
    {
        ...
        if (tunings.count("lw_step_1_rt") == 0)
        {
            ...
            std::tie(grid_1, block_1) = tune_kernel(
                    ...
                    ncol, nlay, eps, top_at_1, ds, weights, tau, lay_source,
                    lev_source, sfc_emis, sfc_src, flux_up_tmp, flux_dn_tmp, sfc_src_jac,
        ...
        apply_BC_kernel_lw<<<grid_gpu1d, block_gpu1d>>>(top_level, ncol, nlay, top_at_1, flux_dn, radn_dn);
        ...
        if (nmus > 1)
        {
            for (int imu=1; imu<nmus; ++imu)
            {
                ...
                /*
                lw_solver_noscat_step_1_kernel<<<grid_1, block_1>>>(
                        ncol, nlay, eps, top_at_1, ds+imu, weights+imu, tau, lay_source,
                           ...
                           Float* flux_up, Float* flux_dn, Float* flux_dir)
    {
        ...
        if (tunings.count("sw_source_2stream_kernel_rt") == 0)
        {
            if (top_at_1)
            {
                std::tie(grid_source, block_source) = tune_kernel(
                        ...
                        ncol, nlay, tau, ssa, g, mu0, r_dif, t_dif,
        ...
        if (top_at_1)
        {
            adding_kernel<1,1><<<grid_adding, block_adding>>>(
                ncol, nlay, top_at_1,
                sfc_alb_dif, r_dif, t_dif,
                source_dn, source_up, source_sfc,
        ...
        {
            adding_kernel<0,1><<<grid_adding, block_adding>>>(
                        ncol, nlay, top_at_1,
                        sfc_alb_dif, r_dif, t_dif,
                        source_dn, source_up, source_sfc,
                        flux_up, flux_dn, flux_dir, albedo, src, denom);
                           ...
                           Float* flux_up, Float* flux_dn)
    {
        ...
        Float* source_dn = Tools_gpu::allocate_gpu<Float>(opt_size);
        Float* source_sfc = Tools_gpu::allocate_gpu<Float>(sfc_size);
        Float* albedo_sfc = Tools_gpu::allocate_gpu<Float>(sfc_size);
        ...
        // Step 1.
        if (tunings.count("lw_source_2stream_kernel_rt") == 0)
        {
            if (top_at_1)
            {
                std::tie(grid_source, block_source) = tune_kernel(
                        "lw_source_2stream_kernel_rt",
                        dim3(ncol, 1),
                        {32, 64, 96, 128, 256, 384, 512, 768, 1024}, {1}, {1},
                        lw_source_2stream_kernel<1>,
                        ncol, nlay, tau, ssa, g,
                        sfc_emis, sfc_src,
                        lay_source, lev_source,
                        r_dif, t_dif,
                        source_up, source_dn,
                        source_sfc, albedo_sfc);
            }
            else
            {
                std::tie(grid_source, block_source) = tune_kernel(
            ...
            tunings["lw_source_2stream_kernel_rt"].first = grid_source;
            tunings["lw_source_2stream_kernel_rt"].second = block_source;
        }
        else
        {
            block_source = tunings["lw_source_2stream_kernel_rt"].second;
        }
        ...
        if (top_at_1)
        {
            lw_source_2stream_kernel<1><<<grid_source, block_source>>>(
                    ncol, nlay, tau, ssa, g,
                    sfc_emis, sfc_src,
                    lay_source, lev_source,
                    r_dif, t_dif,
        ...
        {
            lw_source_2stream_kernel<0><<<grid_source, block_source>>>(
                    ncol, nlay, tau, ssa, g,
                    sfc_emis, sfc_src,
                    lay_source, lev_source,
                    r_dif, t_dif,
        ...
        Float* flux_dir_empty;

        if (top_at_1)
        {
            adding_kernel<1,0><<<grid_adding, block_adding>>>(
                ncol, nlay, top_at_1,
                albedo_sfc, r_dif, t_dif,
                source_dn, source_up, source_sfc,
                flux_up, flux_dn, flux_dir_empty, albedo, src, denom);
        }
        else
        {
            adding_kernel<0,0><<<grid_adding, block_adding>>>(
                        ncol, nlay, top_at_1,
                        albedo_sfc, r_dif, t_dif,
                        source_dn, source_up, source_sfc,
                        flux_up, flux_dn, flux_dir_empty, albedo, src, denom);
